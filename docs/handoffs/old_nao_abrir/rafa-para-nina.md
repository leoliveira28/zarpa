...
## S16 — 1. `PropostaParada` agora traz o WhatsApp do contato

`obterResumoDoMes()` → `paradas.itens[]` (`src/server/dashboard.ts`, tipo `PropostaParada`)
ganhou **um campo novo, aditivo**. Nada mais mudou nesse contrato.
...
type PropostaParada = {
  ...
  contactId: string;
  contactName: string;
  contactWhatsapp: string | null; // ← NOVO
...
**Formato exato de `contactWhatsapp`:** o valor **cru**, exatamente como a agente digitou no
cadastro do contato (`contacts.whatsapp`). Pode vir `(11) 98888-7777`, `11988887777`,
`+55 11 98888-7777`, com espaço sobrando, ou `null` se ela nunca preencheu. **O servidor não
normaliza, não valida e não formata** — mesma disciplina que já vale para `contactWhatsapp`
em `listarEmViagem` (`viagens.ts`), e é de propósito: sanitizar aqui criaria uma segunda
regra de formato competindo com o `waMeLink` que você já usa no cliente, e a agente veria um
número "consertado" diferente do que ela digitou.

Do seu lado, no botão "Cobrar" de `/hoje`, continua valendo o mesmo padrão de "Em viagem":
`waMeLink(contactWhatsapp)` e, quando o campo é `null` ou não vira link válido, o botão
segue só copiando a mensagem — que é o comportamento de hoje. Um contato sem WhatsApp não
pode virar botão morto.

## S16 — 2. O funil configurável está LIGADO no banco (`deals.stage_id`)

`drizzle/0016_negocio_aponta_para_estagio.sql`. `deals` passou a ter `stage_id NOT NULL`,
FK para `pipeline_stages` — ou seja: **um negócio já consegue apontar para uma coluna que a
agente criou.** É o que faltava para a sua tela de configuração do funil ter efeito real.

`deals.stage` (o enum de 6 valores) **continua existindo** e continua correto: virou uma
projeção de `stage_id`, mantida por um trigger no banco. Nenhuma chamada sua quebrou, e
`/funil` funciona hoje exatamente como funcionava.

### O que é fonte de verdade agora

| Pergunta | Onde está a resposta |
|---|---|
| Em que coluna o negócio está | `deals.stage_id` → `pipeline_stages` (`stageId`/`stageLabel`) |
| Fechou como ganho? | `pipeline_stages.is_won` (`isWon`) |
| Fechou como perdido? | `pipeline_stages.is_lost` (`isLost`) |
| Quais colunas existem, na ordem | `listarEstagios()` |

`stage` (o enum) só serve de compatibilidade. **Negócio numa coluna criada pela agente sai
com `stage: 'negociando'`** — é o espelho que o banco dá para uma coluna sem equivalente no
enum. Se você continuar montando o quadro por `COLUNAS_DO_FUNIL`, esses cards vão aparecer
embaixo de "Negociando". Não é bug: é o preço de a tela ainda ler a lista fixa. Some no
momento em que `/funil` passar a montar as colunas com `listarEstagios()` e posicionar o
card por `stageId`.

### `listarNegociosDoFunil()` — três campos novos, nada removido
...
type NegocioDoFunil = {
  ...
  destination: string | null;
  valueCents: number;
  stage: EstagioDeFunil;   // continua (projeção; 'perdido' nunca aparece)
  stageId: string;         // ← NOVO — a coluna de verdade
  stageLabel: string;      // ← NOVO — o rótulo que a agente deu
  stagePosition: number;   // ← NOVO — a posição no quadro
  contactId: string;
  contactName: string;
...
O filtro de "não mostra perdido" passou a ser por `is_lost` da coluna, não pelo literal
`'perdido'`. Ou seja: se a agente renomear "Perdida" para "Não rolou", o card continua
saindo do quadro.

### `moverEstagioDoNegocio` — aceita os DOIS destinos

```ts
type DestinoDeEstagio = DealStage | { stageId: string };
...
- **Toda chamada que você já tem continua idêntica.** `moverEstagioDoNegocio(id, 'ganho')`,
  `(id, 'perdido', motivo)`, `(id, previous.stage)` — nada a mudar.
- Novo: `moverEstagioDoNegocio(id, { stageId })` move para qualquer coluna ativa do funil,
  inclusive uma criada pela agente. É o que o arrasto do quadro configurável deve chamar.
- **Motivo de perda:** continua obrigatório (mín. 3 caracteres depois de `trim`), mas a
  regra agora é *"a coluna de destino é `is_lost`"*, não *"o enum é `'perdido'`"*. Renomear
  a coluna não desliga a exigência. Erro: `DADOS_INVALIDOS` / `campo: 'motivoPerda'`.
- Coluna **arquivada** como destino: `CONFLITO` / `campo: 'stageId'` — "Essa coluna está
  arquivada."
- Coluna de **outro tenant** (ou id que não existe): `NAO_ENCONTRADO`. Provado com SQL cru
  também: o RLS derruba antes de qualquer coisa ser gravada.
- Idempotência continua: mover para a coluna onde o negócio já está não grava nada, não
  duplica `activity` nem `audit_log` — agora a comparação é por `stage_id`.

Retorno (`NegocioMovido`) ganhou campos, nenhum saiu:

```ts
type NegocioMovido = {
  id: string;
  stage: DealStage;      // projeção
  stageId: string;       // ← NOVO
  stageLabel: string;    // ← NOVO
  isWon: boolean;        // ← NOVO
  isLost: boolean;       // ← NOVO
...
**A timeline não mudou de formato.** `activities.body` e `metadata.de`/`metadata.para`
continuam em valor de ENUM (`"Movido de novo para cotando."`, `{ de: 'novo', para:
'cotando' }`) exatamente porque `NegocioScreen.tsx` reconstrói a frase a partir dessa
metadata com `STAGE_LABEL` e monta a versão otimista no mesmo formato — eu não ia quebrar
sua linha do tempo sem avisar. A metadata **ganhou** `deStageId`, `paraStageId` e
`paraLabel`; quando quiser exibir o rótulo de verdade (inclusive de coluna customizada, que
o `STAGE_LABEL` não conhece), leia `paraLabel` e caia para `STAGE_LABEL[para]` como
fallback.

### `criarNegocio` — `stageId` opcional

```ts
type CriarNegocioInput = {
  contactId: string;
  title: string;
  // ...tudo o que já existia, sem mudança...
  stageId?: string; // ← NOVO, opcional
};
```

- **Omitido = comportamento de hoje**: nasce em "Novo contato" (o `legacyStage: 'novo'`).
  Isso continua valendo mesmo para conta que nunca configurou o funil.
- Com `stageId`: nasce na coluna escolhida. Recusa em dois casos, os dois com `CONFLITO`:
  coluna **arquivada**, e coluna de **fim de funil** ("Um negócio não nasce fechado" —
  ganho exige a venda, perdido exige motivo de perda, e nenhum dos dois cabe no formulário
  de criação; crie aberto e mova).
- O retorno é o `NegocioDoFunil` completo, já com `stageId`/`stageLabel`/`stagePosition`.

### `obterNegocio` / `NegocioDetalhe`

Ganhou `stageId`, `stageLabel`, `isWon`, `isLost` (aditivo). Use `isWon` no lugar de
`negocio.stage === "ganho"` para decidir se mostra o card de roteiro — assim continua certo
quando a agente renomear "Fechada".

### `listarEstagios()` — o que sua tela de configuração vai consumir

Sem mudança de assinatura, mas duas coisas melhoraram e importam para você:

- `totalNegocios` agora conta por `stage_id`. Antes, coluna criada pela agente contava
  sempre 0 (não tinha como ter negócio). Agora o número é real — e é ele que decide se o
  botão de arquivar pode aparecer.
- **Nunca devolve lista vazia.** Conta que nasceu fora do cadastro normal recebe o funil de
  fábrica na hora da leitura. Você pode assumir que sempre existem colunas, sempre exatamente
  uma `isWon` e uma `isLost`.

`criarEstagio`, `renomearEstagio`, `reordenarEstagios`, `arquivarEstagio`: assinaturas
intactas (S15). Lembretes que agora têm efeito prático: `arquivarEstagio` recusa fim de
funil e recusa coluna com negócio dentro (`CONFLITO`, com a contagem na mensagem);
`reordenarEstagios` exige a lista **completa** dos ids ativos.

### O que eu NÃO fiz, de propósito, e é seu

`/funil` continua montando as colunas por `COLUNAS_DO_FUNIL` (`src/server/dealStages.ts`).
Não troquei por `listarEstagios()` porque a troca é toda dentro de `src/app/**` /
`src/components/**` — sua fronteira, e você vai reescrever essa tela na sequência. O backend
já entrega tudo que a troca precisa. Quando fizer: monte as colunas com `listarEstagios()`,
posicione o card por `stageId`, e no arrasto chame `moverEstagioDoNegocio(id, { stageId })`.
